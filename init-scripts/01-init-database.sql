-- ============================================================================
-- DATA1500 - Oblig 1: Initialiserings-skript
-- ============================================================================
...
CREATE TABLE kunde (
    ...
    CONSTRAINT check_mobilnr CHECK (mobilnr ~ '^\+?[0-9]{8,15}$'),
    CONSTRAINT check_epost   CHECK (epost ~ '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$')
);
...
CREATE TABLE utleie (
    ...
    belop               NUMERIC(10,2),
    CONSTRAINT check_tidspunkt CHECK (innlevert_tidspunkt IS NULL OR innlevert_tidspunkt > utleie_tidspunkt),
    CONSTRAINT check_belop     CHECK (belop IS NULL OR belop >= 0)
);
...
INSERT INTO utleie (kunde_id, sykkel_id, start_stasjon_id, slutt_stasjon_id, start_las_id, slutt_las_id, utleie_tidspunkt, innlevert_tidspunkt, belop)
SELECT
    (n % 10) + 1,
    (n % 100) + 1,
    (n % 5) + 1,
